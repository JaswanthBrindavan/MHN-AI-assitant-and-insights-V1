...
`pg`-marked tests and `tests/test_coexistence.py` is not among them — it skips
at module level because `db/existing_schema.sql` is absent. CI checks out this
repo only, so the job that was supposed to run it cannot.

That check is the one that would have caught V18. Making it skip everywhere
means it currently protects nothing. `test_migrations.py` and
`test_pg_hybrid_retrieval.py` still run in CI; the parity check skips too.

This is a direct consequence of gitignoring `db/`, which was the right call for
a different reason — mhn-spring owns those files and two copies drift. The
options, none of which I took on my own:

1. **Commit `db/existing_schema.sql` alone** (not the Flyway files). It is a
   generated artefact of *their* chain, which is exactly what makes it awkward
   to own — but it is also the only input the check needs.
2. **Give CI read access to mhn-spring** and regenerate the dump in the job
   (`python -m scripts.build_existing_schema`). Correct, and it makes the check
   track their chain automatically; needs a token and their consent.
3. **Accept it as a local pre-merge ritual** and say so out loud, so nobody
   believes CI is covering it.

Until one is chosen, treat "the coexistence check passes" as a claim nobody has
verified since the dump was last regenerated by hand.
