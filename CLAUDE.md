...
> ## ⇢ NEXT SESSION: READ `project_docs/handover.md` FIRST
>
> Its **START HERE** block is the ordered work list, and it is not the same
> backlog this file used to describe. In brief:
>
> 0. **The nightly sweep has never run** — `job_runs` is empty. So no erasure
>    request has ever completed, retention has never run, and
>    `user_memory_document` is empty. Trigger it by hand and watch the first
>    run: it clears the whole backlog at once.
> 1. **Plain-language emergency recall** (audit C1, C2).
> 2. **The output guards** — three of them are thinner than this file implies
>    (C3, C4, C7).
> 3. **The one-line correctness batch** (D1, D2, S10, S3, R1, R6) — R1 is a
>    production LLM call with *no timeout*.
> 4. **`APP_ENV` defaults to `"dev"`**, disarming the refuse-to-start guard (S1).
> 5. **Observability** — there is no logging config at all, and nothing scrapes
>    `/metrics` (R7, R8).
>
> Full detail and 80 verified findings: `project_docs/audit.md`, which replaced
> `drawbacks.md`. Read its correction box before acting on any severity — two
> findings were re-measured by hand and both moved.
...
**mhn-spring's head is V34, so the next Davi migration is V35+**.
...
there against a Homebrew Postgres 16 with a source-built pgvector — see the
`local-dev-postgres` memory. On the current machine Docker IS available:
`docker run -d --name davi-coexist-pg -e POSTGRES_USER=davi -e
POSTGRES_PASSWORD=davi -e POSTGRES_DB=davi -p 5433:5432 pgvector/pgvector:pg16`
(port 5433 — 5432 is taken by a live mhn-ai database), then
...
A deterministic safety handler that a message can route AROUND. Four instances
so far, three found by using the deployed app rather than by reading code:

1. The drug-interaction refusal sat inside the legacy branch; the agentic
   engine skipped it entirely.
2. `episodes.worst_level` was written to escalate an unresolved red flag and
   was never called from anywhere.
3. `extract_interaction_query` cleaned "my bp tablet" down to `bp`, hit a
   3-character minimum, and returned None — so the refusal never fired and the
   model answered an interaction question from its own weights.
4. `SELF_HARM_PHRASES` contained a bare `"been cutting"`, so "I have been
   cutting down on sugar" was an EMERGENCY with the self-harm crisis reply.

The pattern: **a guard whose reachability depends on parsing succeeding.**
When adding one, ask what phrasing reaches the shape but not the guard — and
test it against the deployed app, because the suite passed cleanly through all
four.
